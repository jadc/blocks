...
function updateClock(){
  ...
  // Styling
  clock.style.transform = `translateX(${offset}em)`;
  if(isCCD()) clock.style.color = "#ffff77";

  // Anti burn-in
  if(offset > 0.5 || offset < -0.5) offsetInc *= -1;
  offset += offsetInc;
...
function isMonday() {
  let today = moment().format(dateFormat);
  let monday = moment().day("Monday").format(dateFormat);
  return (today == monday) ? true : false;
}
